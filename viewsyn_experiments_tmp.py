...
def load_test_data():
	img = []
	# im = Image.open("../data/tmp/tmp1/0_0.png").convert('RGB')
	# img.append(np.asarray(im))
	for filename in os.listdir("../data/tmp/tmp1/"):
		im = Image.open("../data/tmp/tmp1/" + filename).convert('RGB')
		img.append(np.asarray(im))

	for filename in os.listdir("../data/tmp/tmp2/"):
		im = Image.open("../data/tmp/tmp2/" + filename).convert('RGB')
		img.append(np.asarray(im))
	return np.array(img)
...
def run_autoencoder(test_images):
	...
	#test autoencoder
	autoencoder.load_weights('../model/weights.99-0.92.hdf5')
