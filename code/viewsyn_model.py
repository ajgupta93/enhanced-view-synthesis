...
def train_autoencoder(autoencoder):
	...
	history = autoencoder.fit_generator(util.generate_autoencoder_data_from_list(dataArr[:int(math.ceil(.8*len(dataArr)))]), samples_per_epoch = 293844, nb_epoch = 300, verbose=1, callbacks=callbacks_list,
		 validation_data=util.generate_autoencoder_data_from_list(dataArr[int(math.ceil(.8*len(dataArr))):]), nb_val_samples=128, class_weight=None, initial_epoch=0)#util.generate_autoencoder_data_from_list(dataArr[int(math.ceil(.8*len(dataArr))):]), nb_val_samples=128
